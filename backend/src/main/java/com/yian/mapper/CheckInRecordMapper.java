...
public interface CheckInRecordMapper extends BaseMapper<CheckInRecord> {
     ...
     */
    @Select("SELECT * FROM check_in_record WHERE resident_id = #{residentId} AND status = 'CHECKED_IN' AND is_deleted = 0 LIMIT 1")
    CheckInRecord selectCurrentByResidentId(@Param("residentId") Long residentId);

    /**
     * 查询某床位当前入住中的记录，用于判断床位是否已被占用。
     */
    @Select("SELECT * FROM check_in_record WHERE bed_id = #{bedId} AND status = 'CHECKED_IN' AND is_deleted = 0 LIMIT 1")
